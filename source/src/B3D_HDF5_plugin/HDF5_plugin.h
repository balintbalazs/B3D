...
size_t H5Z_cudaCompress_filter(unsigned flags, size_t cd_nelmts, const unsigned int cd_values[], size_t nbytes, size_t *buf_size, void **buf);

DLL int initDirectCudaCompress(const size_t* size, int dwtLevels, float quantStep, GPUResources** res);
DLL int closeDirectCudaCompress(GPUResources** res);
DLL int directCudaCompress(hid_t dset_id, hsize_t* offset, size_t* size, void* data, uint dwtLevels, float quantStep, float bgLevel, int tileSize, int onDevice, GPUResources** res);
