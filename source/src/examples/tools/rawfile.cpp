...
#if defined(_WIN32)
#define byteswap_ushort(x) _byteswap_ushort (x)
#elif defined(__gnu_linux__)
#define byteswap_ushort(x) __builtin_bswap16 (x)
#endif
...
bool readSwapShortRaw(const std::string& filename, uint elemCount, short* pResult)
{
	std::ifstream file(filename.c_str(), std::ios_base::binary);
	if (!file.good())
		return false;

	size_t bytes = elemCount * sizeof(unsigned short);
	file.read((char*)pResult, bytes);
	size_t bytesRead = file.gcount();
	file.close();

	for (size_t i = 0; i < elemCount; i++) {
		pResult[i] = byteswap_ushort(pResult[i]);
	}

	return bytes == bytesRead;;
}

bool writeSwapShortRaw(const std::string& filename, uint elemCount, const short* pData)
{
	short* buff;
	buff = new short[elemCount];

	for (size_t i = 0; i < elemCount; i++) {
		buff[i] = byteswap_ushort(pData[i]);
	}
	...
	file.write((char*)buff, elemCount * sizeof(short));
